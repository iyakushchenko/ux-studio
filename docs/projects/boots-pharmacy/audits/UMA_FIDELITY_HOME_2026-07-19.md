...
**Surface:** Boots Pharmacy Site Pilot (`screenId: site-pilot`, Frame child **11**, Legacy `Body10`)  
**Date:** 2026-07-19  
**Owner:** Uma (UI/UX)  
...
**Legacy truth:** `frame/index.tsx` `Body10` · `ComponentCoOrderSummary9` · `Subtotal11` · `ComponentInputButton4` (mic) · `ComponentInputButton5` (send) · `Frame351`/`Frame352` (suggested chips) · `globals-screens` child-11 · `globals-chrome` primary/mic hover  
**Register:** [../features/HOME_LEGACY_PARITY_REGISTER.md](../features/HOME_LEGACY_PARITY_REGISTER.md) — align H-bands → L-bands  
...
|-------|-------|
| **Overall** | **PROVEN** |
| **§0a typical DS / pointer matrix** | **PASS** — mic · send · chips · textarea (Legacy CSS in `site-pilot-composer.css`; Quinn MCP hover steps) |
...
**Honest scope (Legacy frame child 11):** unchanged — no footer / crumbs / Advantage invent.

---

## Layout bands — Arch Body10 inventory (await Bea register)

| # | Legacy band / component | Legacy truth (computed / DOM) | Uma stamp | Notes |
|---|------------------------|----------------------------|-----------|-------|
...
| **H2** | **Page bg / atmosphere** — full-bleed body, no footer seam | Legacy export `bg-[#f5f5f5]`; wire override **gradient** `#c5dde8 → #dbebf5 → #e8f3f9` on `[data-name="body"]`; decorative Figma layers **hidden** in wire | **PENDING** | FAIL class: flat white substituting gradient; invent decorative PNG |
...
| **H8** | **Mic icon button** — `ComponentInputButton4` | **48×48** circle · white bg · border `#efefef` · mic glyph `#3A3A3A` | **PENDING** | §0a: default → hover → active per Legacy LEGACY (mint wash `#eef8f7`, border `#afccca`, glyph `#012169`, active `#c6e5e1`) |
...
| **H11** | **Suggested chips (×3)** — `component.gse.system.message` | Rest `rgba(204,224,242,0.57)` · **rounded 16** · 13px `#012169` · labels: Vaccine / Skin health / Other Health services | **PENDING** | §0a: hover `rgba(175,204,202,0.55)` + shadow + translateY · active deeper mint — cite Legacy CSS, **no invent** |
| **H12** | **Vertical rhythm — hero stack** | Logo → heading → card **`gap-[72px]`** on Legacy column | **PENDING** | MCP measure before layout PARTIAL (§0b spirit) |
...
**Hard rule:** Rest-state green + missing hover = **FAIL**. Invent hover not in Legacy / UXDS kit = **FAIL**.

| Control | States to prove (Legacy + kit) | Status | Evidence |
|---------|------------------------------|--------|----------|
| **Textarea** (`agentic-home-query`) | default · filled · focus (outline none per wire) · placeholder · multiline growth 1–5 lines | **PENDING** | — |
| **Mic** (48px circle, Vector glyph) | default · **hover** · **active** · focus-visible | **PENDING** | Legacy: `globals-screens` child-11 mic block |
| **Send** (primary pill, arrows glyph) | default **`#012169`** rest · **hover** lift · **active** press · disabled N/A | **PENDING** | Legacy: `globals-chrome` `bg-[#003fcc]` catch + hover `#01318f` |
| **Suggested chips (×3)** | default · **hover** · **active** · focus (keyboard) | **PENDING** | Legacy: `globals-screens` chip hover block |
...
| Promo / Advantage / AI strips | **N/A** | **Verified absent** on Home Legacy frame — do not port PLP/PDP bands |
...
| Icon-only mic | **PENDING** | Circular wash per Legacy — not fuchsia invent |
| Borders / radii / shadows | **PENDING** | Card 16 · chips 16 · mic border `#efefef` only |
| Icon+text nowrap | **N/A** | Chips single-line labels only |
| TertiaryCta pattern | **N/A** | Not used on Home Legacy |

---

## Side-by-side screenshot pass (PENDING)

- [ ] First viewport Legacy vs React (header + hero + card)
...
| Invent chip/send/mic hover colors | **FAIL** | Port Legacy LEGACY values only — under-match beats invent |
...
| Footer / breadcrumbs / Advantage strip on Home | **FAIL** | Legacy frame has header + `Body10` only |
| PromoMessageStrip or TertiaryCta forced reuse | **FAIL** | **N/A on Home** — mic/send are `component.input.button` |
| Flat `#fff` body vs Legacy gradient atmosphere | **FAIL** | H2 gradient + hidden decorative layers policy |
...
| LEGACY `globals-screens` child-11 rules not applied on React mount | **FAIL** | Hybrid retire Legacy body; port interactions to page CSS / kits |
...
| Line | Stamp |
|------|-------|
| `loading states` | **N/A** — no Legacy loader; invent = FAIL |
...
Uma (UI/UX): loading states — N/A (no Legacy loader; invent = FAIL)
...
Uma (UI/UX): PromoMessageStrip / Advantage — N/A (verified absent on Home Legacy Body10)
Uma (UI/UX): TertiaryCta — N/A (Home uses component.input.button mic/send only)
```

**Knowledge used:** UMA_FIDELITY_NOTES §0/§0a/§0b (hero rhythm measure) · VISUAL_FIDELITY §1 design-delta · PAGE_FINAL_PASS sequencing · LESSONS invent-vs-Legacy + DS hover · Arch Body10 inventory (`frame/index.tsx`) · PDP stamp format — **no PROVEN claim**.
...
- [PLP_LEGACY_PARITY_REGISTER.md](../features/PLP_LEGACY_PARITY_REGISTER.md) — register format reference (Bea to file `HOME_LEGACY_PARITY_REGISTER.md`)  
