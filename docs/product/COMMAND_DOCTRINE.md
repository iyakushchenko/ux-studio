...
**Hard rules:** After a big ship, Arch runs **team check** before declaring done — tests green alone do not skip it. Ship **cannot** be “done” if **Uma (UI/UX)** or **Quinn (QA)** reports **FAIL**. Quinn **cannot PASS** if the Make-parity register still has unchecked P0s. Wrong/missed Make loading scenario (e.g. blank + “Updating results…” alone) or missing checkbox/radio hover = automatic fail.
