# Uma (UI/UX) — Legacy → React fidelity checklist
...
**Programmatic contracts:** Typical Make→React misses are also gated by `npm run check:parity-ratchets` ([PARITY_RATCHETS.md](./PARITY_RATCHETS.md)) — search icons, **search-field-states** (kit `:hover`/`:focus-within`), bookmark copy, empty-heart fuchsia, Advantage bar, Book now primary, loader dup, legacy-retired. Uma checklist still required for pixels/hover; ratchets catch the classes we keep burning. DS state checks = rule of thumb for every interactive kit.

**Bea (BA)** must list every Legacy band/component in the parity register **before** Finn codes — including **loading / empty / updating** states as **P0** rows when Legacy has them. Missing whole components (see Advantage Card bar on PLP) = ship fail.
...
Run against Legacy (or Legacy frame export) **side-by-side** with React localhost.

### 0a. Typical DS state matrix (RULE OF THUMB — every field / button)

**Hard rule:** Before any screen is **PROVEN**, Uma walks the **full interactive state matrix** for every field and button on that screen — against **UXDS kit CSS** + **Legacy**, not rest-state only. Rest-state green + missing hover = **FAIL** (PO: SearchField had no hover).

For **each** field (`SearchField` / text input) and **each** button (primary / secondary / tertiary / icon-only):

| State | Must verify |
|-------|-------------|
| **default** | Border / fill / type / icon color match kit + Legacy |
...
| **focus** / focus-within | Same ring language as Legacy; keyboard visible |
| **active** / pressed | CTA pressed fill when Legacy has it |
| **filled** | Value + clear affordance; icon position unchanged |
| **disabled** | Dim / non-interactive; no hover flash |
| **error** | Only if Legacy/DS shows error on that control |
...
- [ ] **FAIL class:** invent hover chrome not in UXDS kit / Legacy = ship fail
...
1. Select the Legacy RTB column (`component.pdp.rtb` / register L6) and the React column (`.pdp__rtb-col`).
2. For each adjacent pair in Legacy stack order — **title block → list price → recipient/CTA row → body blurb → booster → Book CTA row** — record:
   - computed parent `gap` (Legacy `gap-[32px]` on ComponentPdpRtb)
...
4. **FAIL class:** cramped hierarchy; parent gap ≠ Legacy; LEGACY CSS (`nth-child` + `data-name`) restyling React mounts (e.g. forcing `gap: 48px` on the column or `aspect-ratio: 1/1` on the title block).
5. **FAIL class:** claiming fidelity IN PROGRESS / PARTIAL layout PASS without citing measured gaps.

- [ ] Parent column `gap` matches Legacy (PDP RTB = **32px**)
- [ ] price → recipient gap measured = Legacy
- [ ] recipient → body gap measured = Legacy
- [ ] body → booster gap measured = Legacy
- [ ] booster → CTA gap measured = Legacy
...
**Hard rule:** Before Finn codes any list/filter/search screen, Uma + Bea must capture the **exact Legacy loading scenario** (skeleton tiles vs spinner overlay vs dimmed list vs shimmer vs text-only). Write it into the parity register as a **P0** row with layout notes.

- [ ] Legacy loading scenario documented (mechanism + copy + where it sits in the layout)
- [ ] Filter/search/pagination change shows that same scenario — not a invented substitute
- [ ] Empty state matches Legacy when zero results
- [ ] **Exactly one** “Updating…” (or equivalent) treatment — spinner overlay label **or** count line, **not both** (duplicate = FAIL)
- [ ] No layout jump from host collapse — lock band height while tiles hide if Legacy feels stable
- [ ] **FAIL class:** blank listing + lone “Updating results…” **without** spinner/overlay/skeleton = ship fail
- [ ] **FAIL class:** invented duplicate loader copy / orange pulsed count-line not matching Legacy = ship fail

**Screenshot notes required:** loading frame (spinner visible in-band), then results return / stagger.

### 1. Page chrome / atmosphere

- [ ] Page background fill / pattern / opacity matches Legacy
- [ ] Shadows / lifts on hero, cards, listing wrappers match Legacy (no invent, no omit)
- [ ] Wrappers / shells / radii match Legacy (no anonymous card borders)
- [ ] Preloaders / skeleton / listing load sims present **and match Make’s mechanism** (see §0)

### 2. Promo / banner / strip bands (scan Legacy for ALL)
...
- [ ] LEGACY `!important` catch-alls do **not** steal React UXDS hover
- [ ] Disabled states match Legacy when present

### 4. Icon buttons — hover + pressed feedback

- [ ] Hearts / bookmarks / wishlist: **exact Legacy empty vs filled** — do **not** invent fuchsia on **empty** hover if Legacy uses tertiary navy
- [ ] **Bookmark label states (PO copy):** not bookmarked → **"Add to Bookmarks"** (empty heart hover = Legacy tertiary navy, not invented fuchsia); bookmarked → default **"In your Bookmarks"**, hover **"Remove from Bookmarks"**
- [ ] Filled/active + click-optimistic only where Legacy does; empty hover follows Legacy tertiary/icon rules
- [ ] Share / eye / other icon-only hits: circular wash or tertiary rules per FE standards
- [ ] No laggy click feedback; **FAIL** if invented hover chrome not in Legacy CSS
- [ ] **Forbidden:** invent hover/loading chrome not in Legacy ([LESSONS_LEARNED.md](./LESSONS_LEARNED.md))

### 5. Checkbox / radio hover (Legacy parity — mandatory sign-off)

- [ ] Unchecked checkbox/radio shows Legacy hover wash (Boots: mint `#c6e5e1` fill + border) on row or control hover
- [ ] Checked state stays Legacy checked colors on hover (no flash to white/grey)
...
- [ ] Borders only if Legacy has them (no invent tile borders)
- [ ] Radii / shadows match Legacy; no DS “pretty” upgrades

### 7. Button types — icon+text vs text-only

- [ ] Icon+text pattern where Legacy/DS uses tertiary compact (e.g. Reset Filters + trash)
- [ ] Text-only links only where Legacy is text-only (`.uxds-link`)
...
- [ ] Magnifier side matches PO/Legacy wire — PLP / Availability / Book Step 1 = **RIGHT** (`data-studio-search-icon-pos="end"`)
- [ ] Magnifier is a **bare glyph** — no box border / inset ring on the icon (Legacy paints the ring on the field’s absolute `[aria-hidden].absolute` overlay only; UXDS `.uxds-search-field__control` owns hover/focus)
...
- [ ] Filter options show Legacy-like **counters** (`data-studio-plp-option-count`)
- [ ] No invented filter accordion horizontal separators

### 8. Side-by-side screenshot pass

- [ ] First viewport Legacy vs React
...
| **Site Pilot Chat** (`chat`) | [../projects/boots-pharmacy/audits/UMA_FIDELITY_CHAT_2026-07-19.md](../projects/boots-pharmacy/audits/UMA_FIDELITY_CHAT_2026-07-19.md) | **IN PROGRESS** — hybrid Legacy + DOM composer; shared kit + motion migration PENDING; **NOT PROVEN** |
