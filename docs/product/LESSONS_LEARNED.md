...
- **CI wall clock** — install dominates; warm `node_modules` cache (skip `npm ci`) + parallel `test`∥`build` + Vitest forks/workers is the free-runner path. Honest warm floor ~18–22s; target ≤20–25s. Do not gut hard gates for speed.
