...
9. [ ] **LEGACY retirement (by screen)** — shrink Make wire + `globals-screens` as React pages land. Concept `.proto-*` classes retire with their screen. **Scoping note (2026-07-23, from Phase B):** `BootsPharmacyProjectView.tsx` has ~10 `useEffect` blocks (~1500+ lines) commented "Make path only" and gated solely by `isBookStep{1,2,3}ReactMounted() → return` — since Book Step 1–3 mount unconditionally (no flag), these guards are always true and the effect bodies are permanently unreachable today. Prime first cut for this item.
