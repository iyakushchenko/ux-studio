...
3e. [x] **PDP PAGE FINAL PASS hard-green** — restored after FAQ Accordion + download CTA ship; structure + UXDS Accordion contract + `check:page-final-pass` green (5 screens); Quinn MCP `__studioRunMcpPageProbe({ screenId:"pdp" })` 22/22 PASS on tip `d6e4951` / v0.0.27 (docs `828ab2b`); Uma §0a PROVEN (`c037d19`); `mcpFinalPass: HARD-GREEN`. Audit: [FE_AUDIT_PDP_PAGE_FINAL_PASS_2026-07-19.md](../projects/boots-pharmacy/audits/FE_AUDIT_PDP_PAGE_FINAL_PASS_2026-07-19.md). **Home sequencing-unblocked — wait PO `+` before start.**
