...
| **Reuse** | Maximize UXDS + internal ready components; when PO asks for a page “from what we have,” compose — don’t invent ([CONCEPT_INTAKE.md](./CONCEPT_INTAKE.md) §5 mode B) |
...
| Imperative DOM scripts for many interactions | React state + small, intentional DOM bridges only where Studio needs them |
...
5. Register screen + `data-*` hooks for cursor, touchpoints, recording.
6. Wire journey beats / scripts as needed.
7. Smoke: browse + one playback path that hits the screen.
