...
**PO ask:** Pain / Worked / Keep after both screens hard-green; lock keep actions before Book Make delete / header lane.
...
**Arch:** Chat was hard-green; we sequenced History then Details per PAGE FINAL PASS. Themes: Make densify `!important` beating React CSS, duplicated account `<aside>`, invent Cancel/Edit hover, per-screen densify truth (32/56 vs 20/20), terminal CTA hide, crumb → **React** History, engine probe registry, playback chain-advance for View Details. Both screens stamped HARD-GREEN 2026-07-22 — this retro was due **before** opening erase-Make Book children; filing it now.
...
**Pain**
- Reflex micro-retro lagged the ship — Knowledge improved stamps on 2026-07-22 said “Reflex before Book Make delete” but the Pain/Worked/Keep doc did not land same wave; process debt until this file.
...
- Header/footer asymmetry remains: footer is Make-free (`footerContent.ts` + `Footer.tsx`); header still `cloneNode(true)` on live Make DOM — frame delete blocked until a dedicated Final-Pass lane (see [NEXT_STEPS.md](./NEXT_STEPS.md) 7c).
...
**Keep**
- After **each** HARD-GREEN page (or sequenced pair close): Arch **micro-retro** → [TEAM_KNOWLEDGE.md](./TEAM_KNOWLEDGE.md) index **before** next erase-Make or chrome lane.
- No next migrated page / Make delete until prior Final Pass hard-green + Reflex filed.
...
**Worked**
- [APPOINTMENT_HISTORY_MAKE_PARITY_REGISTER.md](../projects/boots-pharmacy/features/APPOINTMENT_HISTORY_MAKE_PARITY_REGISTER.md) + Details register gave Quinn/Uma shared P0 rows (View Details selector HARD, breadcrumb back, terminal rule).
...
- Details back-nav HARD: crumb “Appointment history” → React `appointment-history` — no Make History handoff after Details ships.
...
**Pain**
- Make densify on History child-2 (`globals-chrome.css`, `!important` + `data-name`) overrode React page CSS until gated — looked like “FE typo,” was global chrome law.
...
**Worked**
- Densify gate `:not([data-studio-react-screen])` on History child-2; Details child-1 stays densified until React mount retires Make.
...
- Deferred unmount + `retireMakeUnderPage` pattern consistent with prior migrations.
...
**Keep**
- Measure **computed** styles vs Make before PROVEN — especially when Make `data-name` survives on React nodes.
...
- View Details handoff exercised Make Details before Details React — intentional sequence; restore History for `url-screen` had to stay in matrix.
...
**Worked**
- History **8/8** including View Details hover/click, Make Details hop, return History; Details **10/10** + terminal CTA hide + crumb back → React History.
- `reload: false` + overlay visible every step — LESSONS R11 `:5173` reuse tab.
- Interaction inventories `readinessPass` true, `invalid` 0 for both Make baseline and React rematch.
...
- Felony/parity gates don’t catch “header still Make clone” — docs lane 7c is the honest gate before frame delete.
...
- Header re-author lane gets same gate discipline as a migrated page before deleting Make frame.
...
**Pain**
- Invent Cancel color and Edit/Cancel hover wash — “tests passed” insufficient until Make-adjacent tertiary pattern applied.
- False comfort when React CSS files showed 32/56 but browser showed 20/20 — PO trust hits computed truth, not source lines.
- Reflex doc missing while board raced toward Book Make delete — sequencing quality vs board optimism.
...
- v0.0.108 local hard-green pair closed erase-Make History/Details acceptance bar.
...
| Theme | Verdict |
|-------|---------|
| **Make densify `!important`** | Gate densify with `:not([data-studio-react-screen])` on retired child; don’t fight in page CSS alone. Measure computed pad/gap/CTA. |
| **Per-screen spacing truth** | Details 20/20 ≠ History 32/56 — register and measure per Make child index. |
...
| **Crumb → React History** | Details breadcrumb “Appointment history” → `INDEX_APPOINTMENT_HISTORY` live React — not Make History. |
...
| **Final-pass sequencing** | History HARD-GREEN before Details mount/Final Pass; Reflex before next erase-Make lane. |
| **Header vs footer** | Footer Make-free; header still Make `cloneNode` — 7c Final-Pass lane before frame delete. |

---

## Top keep actions (Arch synthesis — apply on Book Make delete + header lane)

1. **Densify gate on React hosts** — `:not([data-studio-react-screen])` on the correct Make child; Uma re-measure before PROVEN; Quinn re-probe after fidelity fix.  
...
5. **HARD-GREEN → micro-retro → next lane** — Reflex into [TEAM_KNOWLEDGE.md](./TEAM_KNOWLEDGE.md); then Book Make delete (8) and header re-author (7c) per board.  
