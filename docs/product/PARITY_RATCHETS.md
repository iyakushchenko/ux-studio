...
| 1e | **filter-counters** | Missing option counts | PLP stamps `data-studio-plp-option-count` + calls `countPlpFacetOption` / `countPlpTypeOption` | PO: restore Legacy counters |
...
| 4c | **agent-teardown-clean** | Sticky overlay / `&modal=` after probe / forceClear | Auto-Rule in `studioAutoRules.ts` + `studioAgentTeardownContract.ts`; stay state omits modal; App never re-applies `state?.modalId`; probe/session `finally` calls `resetStudioAfterAgentTest`; `check:felonies` §9 + `__studioAssertAgentTeardownClean` | PO: sticky choose-pharmacy |
| 4d | **avail-logged-out-start** | Logged-out Check availability opens Choose Date | `resolveAvailIntent` returns `{ step: "start" }` when logged-out + no location; unit test + SSoT `isStudioLoggedIn` | PO: Find Pharmacy first |
| 4e | **pdp-rtb-rhythm** | Uma §0b RTB cramped / LEGACY steal | `.pdp__rtb-col` `gap: 32px` (+ host `!important`); LEGACY `module.pdp.rtb` excludes `.pdp__rtb-card` | PO: RTB vertical rhythm |
| 4f | **theme-brand-active** | Active pills use UXDS default teal, not Boots primary | `check:theme-brand` — theme remaps `--uxds-filter-chip-surface-selected-strong` → `var(--project-brand-primary)`; kit uses tokens | PO: brand-active pills |
...
| 8 | **legacy-retired** | Legacy chrome still visible under React | Mount files must set `data-studio-legacy-retired` / `dataset.studioLegacyRetired` | PLP/Book mounts |
...
| 10 | **search-field-states** | SearchField rest-only (no hover ring) | `search-field.css` must define `.uxds-search-field__control:hover` + `:focus-within` with `border-color` / `box-shadow` (Legacy inset ring on shell, not icon) | Uma + Legacy Availability hover |
...
| `data-studio-plp-option-count="<n>"` | Filter option counter (Legacy sidebar) |
...
| `data-studio-legacy-retired="<screenId>"` | Legacy Frame child hidden after React mount |
