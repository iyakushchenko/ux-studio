...
export async function runSelectLocationStore(
  ...
  const chooseBtn = Array.from(storeCard.querySelectorAll<HTMLElement>("button")).find(
    (btn) => /choose location/i.test((btn.textContent ?? "").replace(/\s+/g, " ").trim())
  );
  ...
  await delay(options?.skip ? 280 : 480);
