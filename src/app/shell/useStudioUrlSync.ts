...
export function useStudioUrlSync(options: StudioUrlSyncOptions): void {
  ...
  // Boot: strip leftovers, apply deep link once, canonicalize (drop legacy mode=).
  useEffect(() => {
    ...
    try {
      ...
      // Write immediately — the reflect effect skips while applyingUrlRef is true,
      // so legacy `mode=` would otherwise stick until an unrelated state change.
      const screenId =
        parsed.screenId ??
        resolveScreenIdFromNav({ hubOpen, current, screens });
  ...
  useEffect(() => {
    const onPopState = () => {
      ...
      try {
        ...
        const screenId =
          parsed.screenId ??
          resolveScreenIdFromNav({ hubOpen: false, current: 0, screens });
        const search = writeStudioUrl({
          projectId: parsed.projectId ?? projectId,
          screenId,
