...
/** L16 — Legacy Frame106 intro copy (864px band). */
...
/**
 * Legacy RTB service blurb (Frame PDP body) — also used as FAQ “How can Boots help?”
 * body (Legacy accordion Description missing for that panel; RTB is same-page Legacy copy).
 */
 ...
 *
 * Legacy `ComponentPdpAccordion` only ships a `Description` node for “Who is at risk?”.
 * Other bodies reuse same-page Legacy strings where they answer the header, or
 * Bea-sourced Boots-plausible copy when Legacy has header-only residuals (PO ask).
 *
 * Default open matches Legacy static frame.
 */
...
export const PDP_ACCORDION_PANELS = [
  {
    ...
    /** Legacy RTB service blurb + booking cue (accordion Description absent in export). */
    ...
    source: "legacy+bea" as const,
  },
  {
    id: "who-is-at-risk",
    title: "Who is at risk?",
    /** Legacy `Description` under ComponentPdpAccordion + brief context. */
  ...
  },
  {
    ...
    /** Legacy appt strip + specs Administration + visit flow (no dedicated Description). */
  ...
  },
  {
    ...
    /**
     * Bea-sourced — Legacy header only; Boots-plausible private-vs-NHS framing.
  ...
  },
  {
    ...
    /** Bea-sourced — Legacy header only. */
  ...
  },
  {
    ...
    /** Bea-sourced — Legacy header only; privacy tone aligned with Boots services. */
